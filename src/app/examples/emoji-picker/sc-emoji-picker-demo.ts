...
import { ScEmojiPicker, ScEmojiPickerTrigger, Emoji } from '../../ui/emoji-picker';
  ...
  template: `
    <div class="space-y-8">
      ...
      <section class="space-y-3">
        ...
        <div class="flex items-start gap-2">
          <div class="flex-1">
            ...
            @if (showInputPicker()) {
              <div class="mt-2">
                <sc-emoji-picker (emojiSelect)="insertEmoji($event)" [maxRecent]="6" />
              </div>
            }
          </div>
        </div>
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground mb-2">Use with a popover for dropdown behavior:</p>
        <div class="flex items-center gap-2">
          <button sc-emoji-picker-trigger></button>
          <span class="text-sm text-muted-foreground">← Click to open (requires popover)</span>
        </div>
      </section>
      ...
      <section class="space-y-3">
        ...
        <div class="inline-flex gap-1 rounded-full border bg-background p-1 shadow-sm">
        ...
        @if (lastReaction()) {
          <p class="text-sm text-muted-foreground">You reacted with: {{ lastReaction() }}</p>
        }
      </section>
      ...
      <section class="space-y-3">
        ...
        <div class="max-w-md rounded-lg border p-4">
          <p class="text-sm">This is a sample message that can have emoji reactions.</p>
  ...
  addReaction(emoji: Emoji): void {
    ...
    if (existing) {
      this.messageReactions.set(
        reactions.map((r) => (r.emoji === emoji.emoji ? { ...r, count: r.count + 1 } : r)),
      );
    } else {
      this.messageReactions.set([...reactions, { emoji: emoji.emoji, count: 1 }]);
    }
