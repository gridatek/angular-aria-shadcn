...
import { DateRange, ScCalendar } from '../../ui/calendar';
    ...
    <div class="space-y-8">
      ...
      <div class="space-y-4">
        ...
        @if (selectedDates().length > 0) {
          <p class="text-sm text-muted-foreground">Selected: {{ formatMultipleDates() }}</p>
        }
      </div>
      ...
      <div class="space-y-4">
        ...
        @if (selectedRange().from) {
          <p class="text-sm text-muted-foreground">Range: {{ formatRange() }}</p>
        }
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="rounded-md border">
          <sc-calendar [(selected)]="selectedWithDisabled" [disabled]="disabledDates()" />
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="rounded-md border">
          <sc-calendar [(selected)]="selectedWithMinMax" [minDate]="minDate" [maxDate]="maxDate" />
        </div>
      </div>
    </div>
  `,
...
export class ScCalendarDemo {
  ...
  readonly selectedRange = signal<DateRange>({ from: undefined, to: undefined });
