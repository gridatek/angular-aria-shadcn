  ...
  template: `
    <div class="space-y-10">
      ...
      <section class="space-y-3">
        ...
        <sc-marquee [duration]="30" [gap]="48">
          @for (brand of brands; track brand) {
            ...
            >
              <span class="text-lg font-semibold text-muted-foreground">{{ brand }}</span>
            </div>
          }
      ...
      <section class="space-y-3">
        ...
        <sc-marquee [duration]="40" [gap]="24">
          @for (testimonial of testimonials; track testimonial.name) {
            <div sc-marquee-item class="w-80 rounded-lg border bg-background p-4 shadow-sm">
              <p class="text-sm text-muted-foreground mb-3">"{{ testimonial.quote }}"</p>
        ...
        <div class="h-48 overflow-hidden rounded-lg border">
          <sc-marquee direction="vertical" [duration]="20" [gap]="12">
            @for (notification of notifications; track notification.id) {
              <div sc-marquee-item class="mx-2 rounded-lg border bg-background p-3 shadow-sm">
      ...
      <section class="space-y-3">
        ...
        <div class="space-y-4">
          <div>
            ...
            <sc-marquee-text text="Fast scrolling text" [duration]="10" separator="→" />
          </div>
          <div>
            <p class="text-sm text-muted-foreground mb-1">Normal (20s)</p>
            <sc-marquee-text text="Normal scrolling text" [duration]="20" separator="→" />
          </div>
          <div>
            <p class="text-sm text-muted-foreground mb-1">Slow (40s)</p>
            <sc-marquee-text text="Slow scrolling text" [duration]="40" separator="→" />
          </div>
        </div>
      </section>
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Hover over the marquee to pause animation</p>
...
export class ScMarqueeDemo {
  ...
  readonly testimonials = [
    ...
    },
    {
      name: 'Bob Smith',
      role: 'Lead Developer',
      quote: 'The best Angular components I have ever used. Highly recommended!',
    },
    {
      name: 'Carol Williams',
