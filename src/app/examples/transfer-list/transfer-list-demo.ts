...
import type { TransferListItem } from '../../ui/transfer-list';
      ...
      <div class="space-y-3">
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="rounded-lg border bg-card p-4">
            <p class="text-sm font-medium mb-2">Source ({{ sourceItems().length }})</p>
          ...
          <div class="rounded-lg border bg-card p-4">
            <p class="text-sm font-medium mb-2">Target ({{ targetItems().length }})</p>
            <ul class="text-xs text-muted-foreground space-y-1">
...
export class TransferListDemoComponent {
  ...
  readonly simpleTarget = signal<TransferListItem[]>([{ id: '5', label: 'Option E' }]);

  onChange(state: { source: TransferListItem[]; target: TransferListItem[] }): void {
