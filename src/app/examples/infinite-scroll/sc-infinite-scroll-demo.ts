  ...
  template: `
    <div class="space-y-8">
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Scroll down to automatically load more items.</p>
        <sc-infinite-scroll
        ...
        <p class="text-sm text-muted-foreground">Loaded {{ basicItems().length }} items</p>
      </section>

      <!-- Custom Loader -->
      <section class="space-y-3">
        <h3 class="text-lg font-semibold">Custom Loader</h3>
        <p class="text-sm text-muted-foreground">Use a custom loading indicator.</p>
        <sc-infinite-scroll
        ...
        >
          ...
          <div sc-infinite-scroll-loader class="flex items-center justify-center py-4 gap-2">
            <div
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Infinite scroll with a grid of cards.</p>
        <sc-infinite-scroll
          class="h-[400px] border rounded-lg"
          [loading]="gridLoading()"
  ...
  loadMoreBasic(): void {
    ...
    setTimeout(() => {
      ...
      if (currentLength >= 50) {
        ...
        this.basicItems.update((items) => [...items, ...this.generateItems(currentLength, 10)]);
      }
  ...
  loadMoreCustom(): void {
    ...
    setTimeout(() => {
      ...
      if (currentLength >= 30) {
        ...
        this.customItems.update((items) => [...items, ...this.generateItems(currentLength, 8)]);
      }
  ...
  loadMoreEnd(): void {
    ...
    setTimeout(() => {
      ...
      if (currentLength >= 15) {
        ...
        this.endItems.update((items) => [...items, ...this.generateItems(currentLength, 5)]);
      }
  ...
  loadMoreThreshold(): void {
    ...
    setTimeout(() => {
      ...
      if (currentLength >= 40) {
        ...
        this.thresholdItems.update((items) => [...items, ...this.generateItems(currentLength, 10)]);
      }
  ...
  loadMoreGrid(): void {
    ...
    setTimeout(() => {
      ...
      if (currentLength >= 24) {
        ...
        this.gridItems.update((items) => [...items, ...this.generateItems(currentLength, 4)]);
      }
