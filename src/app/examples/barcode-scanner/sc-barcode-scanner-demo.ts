...
import { ScBarcodeScanner, BarcodeResult, BarcodeFormat } from '../../ui/barcode-scanner';
    ...
    <div class="space-y-8">
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">
          Scan barcodes using your device's camera. Supports QR codes, EAN, UPC, and more.
        </p>
        ...
        @if (lastScannedBasic()) {
          <div class="p-4 bg-muted rounded-lg max-w-md">
            ...
            <p class="font-mono text-sm break-all">{{ lastScannedBasic()?.rawValue }}</p>
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Scanner configured to detect only QR codes.</p>
        <sc-barcode-scanner
          [formats]="['qr_code']"
      ...
      <section class="space-y-3">
        ...
        @if (lastProduct()) {
          <div class="p-4 bg-muted rounded-lg max-w-md">
            ...
            <p class="text-xs text-muted-foreground mt-1">Format: {{ lastProduct()?.format }}</p>
          </div>
        }
      </section>
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Stops scanning after detecting one barcode.</p>
        <sc-barcode-scanner
          [continuous]="false"
        ...
        />
        @if (singleResult()) {
          <div class="p-4 bg-green-50 border border-green-200 rounded-lg max-w-md">
            <p class="text-sm text-green-800 mb-1">Scan complete!</p>
            <p class="font-mono text-sm break-all text-green-900">{{ singleResult()?.rawValue }}</p>
          </div>
        }
      </section>
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Keep track of all scanned barcodes.</p>
        <sc-barcode-scanner
        ...
        @if (scanHistory().length > 0) {
          <div class="max-w-md">
            <div class="flex items-center justify-between mb-2">
              <p class="text-sm font-medium">Scanned ({{ scanHistory().length }})</p>
              <button
                (click)="clearHistory()"
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">The Barcode Detection API is supported in:</p>
...
export class ScBarcodeScannerDemo {
  ...
  readonly productFormats: BarcodeFormat[] = ['ean_13', 'ean_8', 'upc_a', 'upc_e'];
