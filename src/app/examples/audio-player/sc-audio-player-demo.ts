...
import { ScAudioPlayer, AudioTrack } from '../../ui/audio-player';
  ...
  template: `
    <div class="space-y-8">
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Full-featured audio player with all controls.</p>
      ...
      <section class="space-y-3">
        ...
        <sc-audio-player [tracks]="sampleTracks" [variant]="'compact'" class="max-w-lg" />
      </section>

      <!-- Minimal Variant -->
      <section class="space-y-3">
        <h3 class="text-lg font-semibold">Minimal Variant</h3>
        <p class="text-sm text-muted-foreground">Simplified player with just the essentials.</p>
        <sc-audio-player
          [tracks]="sampleTracks"
          [variant]="'minimal'"
          [showCover]="false"
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Player without album artwork display.</p>
        <sc-audio-player [tracks]="sampleTracks" [showCover]="false" class="max-w-md" />
      </section>

      <!-- Single Track -->
      <section class="space-y-3">
        <h3 class="text-lg font-semibold">Single Track</h3>
        <p class="text-sm text-muted-foreground">Player with a single audio file.</p>
        <sc-audio-player [tracks]="[singleTrack]" [showShuffle]="false" class="max-w-md" />
      </section>

      <!-- Custom Colors -->
      <section class="space-y-3">
        <h3 class="text-lg font-semibold">With Podcast</h3>
        <p class="text-sm text-muted-foreground">Works great for podcasts too.</p>
      ...
      <section class="space-y-3">
        ...
        <p class="text-sm text-muted-foreground">Control playback state externally.</p>
        <sc-audio-player
          [tracks]="sampleTracks"
      ...
      <section class="space-y-3">
        ...
        <ul class="text-sm space-y-1 list-disc list-inside text-muted-foreground">
          ...
          <li>
            <kbd class="px-1 py-0.5 bg-muted rounded text-xs">Shift</kbd> + Arrow - Seek 10 seconds
          </li>
        </ul>
        <sc-audio-player [tracks]="sampleTracks" class="max-w-md" />
      </section>
