    ...
    <div class="space-y-8">
      ...
      <div class="space-y-4">
        ...
        <div class="max-w-md space-y-2">
          ...
          <p class="text-sm text-muted-foreground">Try adding the same tag twice.</p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="max-w-md space-y-2">
          <div sc-tag-input [(tags)]="delimiterTags" [delimiters]="['Enter', ' ', 'Tab']">
            @for (tag of delimiterTags(); track tag) {
              <span sc-tag-input-tag [tag]="tag"></span>
            }
            <input sc-tag-input-field />
          </div>
          <p class="text-sm text-muted-foreground">Press Space or Tab to add tags.</p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="max-w-md space-y-2">
          ...
          <p class="text-sm text-muted-foreground">Click outside to add the current input.</p>
        </div>
      </div>
      ...
      <!-- Validation -->
      <div class="space-y-4">
        <h3 class="text-sm font-medium">With Validation (Min: 2, Max: 15 chars)</h3>
        <div class="max-w-md space-y-2">
          <div sc-tag-input [(tags)]="validatedTags" [minLength]="2" [maxLength]="15">
            @for (tag of validatedTags(); track tag) {
              <span sc-tag-input-tag [tag]="tag"></span>
            }
            <input sc-tag-input-field />
          </div>
          <p class="text-sm text-muted-foreground">Tags must be 2-15 characters long.</p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="max-w-md space-y-4">
          <div class="space-y-2">
            ...
            <div sc-tag-input [(tags)]="skillTags" [maxTags]="10" placeholder="Add a skill...">
        ...
        <div class="max-w-lg space-y-2">
          ...
          <div sc-tag-input [(tags)]="emailTags" placeholder="Add recipient...">
            @for (tag of emailTags(); track tag) {
              <span sc-tag-input-tag [tag]="tag" variant="outline" class="rounded-full"></span>
            }
            <input sc-tag-input-field [addOnBlur]="true" />
          </div>
        </div>
...
export class ScTagInputDemo {
  ...
  readonly emailTags = signal<string[]>(['alice@example.com', 'bob@example.com']);
}
