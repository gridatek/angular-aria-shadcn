...
import { ScTabs, ScTabsContent, ScTabsList, ScTabsTrigger } from '../../ui/tabs';

@Component({
  selector: 'app-sc-tabs-demo',
  imports: [ScTabs, ScTabsContent, ScTabsList, ScTabsTrigger],
  template: `
    <div class="flex flex-col gap-8">
      <!-- Basic Tabs -->
      <div sc-tabs [value]="'account'" class="w-[400px]">
        <div sc-tabs-list class="grid w-full grid-cols-2">
          <button sc-tabs-trigger value="account">Account</button>
          <button sc-tabs-trigger value="password">Password</button>
        </div>
        <div sc-tabs-content value="account">
      ...
      <div sc-tabs [value]="'overview'" class="w-[500px]">
        <div sc-tabs-list>
          <button sc-tabs-trigger value="overview">Overview</button>
          <button sc-tabs-trigger value="analytics">Analytics</button>
          <button sc-tabs-trigger value="reports">Reports</button>
          <button sc-tabs-trigger value="notifications" [disabled]="true">Notifications</button>
        </div>
        <div sc-tabs-content value="overview">
          <p class="text-sm text-muted-foreground">
            Overview content. View your dashboard summary and key metrics.
          </p>
        </div>
        <div sc-tabs-content value="analytics">
          <p class="text-sm text-muted-foreground">
            Analytics content. Dive deep into your data and discover insights.
          </p>
        </div>
        <div sc-tabs-content value="reports">
          <p class="text-sm text-muted-foreground">
            Reports content. Generate and download detailed reports.
          </p>
        </div>
        <div sc-tabs-content value="notifications">
          <p class="text-sm text-muted-foreground">
