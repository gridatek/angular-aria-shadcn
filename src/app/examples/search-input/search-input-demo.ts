...
import type { SearchSuggestion } from '../../ui/search-input';
    ...
    <div class="flex flex-col gap-8">
      ...
      @if (selectedItem()) {
        <div class="rounded-lg border bg-card p-4 max-w-md">
          <p class="text-sm">
            Selected: <span class="font-medium">{{ selectedItem()?.label }}</span>
          </p>
          @if (selectedItem()?.description) {
            <p class="text-xs text-muted-foreground">{{ selectedItem()?.description }}</p>
          }
        </div>
      }
    </div>
...
export class SearchInputDemoComponent {
  ...
  readonly categorizedSuggestions: SearchSuggestion[] = [
    ...
    { id: '2', label: 'iPhone 15', description: 'Smartphone', category: 'Electronics' },
    { id: '3', label: 'AirPods Pro', description: 'Earbuds', category: 'Electronics' },
    { id: '4', label: 'Nike Air Max', description: 'Running shoes', category: 'Footwear' },
    { id: '5', label: 'Adidas Ultraboost', description: 'Running shoes', category: 'Footwear' },
    { id: '6', label: "Levi's 501", description: 'Jeans', category: 'Clothing' },
  ];
  ...
  onAsyncSearch(query: string): void {
    ...
    setTimeout(() => {
      this.asyncSuggestions.set(
        this.suggestions.filter((s) => s.label.toLowerCase().includes(query.toLowerCase())),
      );
