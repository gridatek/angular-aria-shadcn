...
import { ScVirtualList, type VirtualListRange } from '../../ui/virtual-list';
    ...
    <div class="space-y-8">
      ...
      <section>
        ...
        <p class="text-sm text-muted-foreground mb-4">
          Rendering {{ items().length.toLocaleString() }} items with virtualization. Only visible
        ...
        <div class="border rounded-lg overflow-hidden">
          ...
          >
            <ng-template let-item let-index="index">
              ...
              >
                <span class="w-16 text-muted-foreground text-sm">{{ index + 1 }}</span>
          ...
          >
            <ng-template let-user let-index="index">
              ...
              >
                ...
                <span class="px-2 py-1 text-xs rounded-full bg-muted">{{ user.role }}</span>
              </div>
            </ng-template>
          </sc-virtual-list>
        </div>
      ...
      <section>
        ...
        <p class="text-sm text-muted-foreground mb-4">Programmatic scrolling to specific items.</p>
        ...
        <div class="border rounded-lg overflow-hidden">
          <sc-virtual-list #controlledList [items]="items()" [itemHeight]="40" height="250px">
            <ng-template let-item let-index="index">
              <div
                class="flex items-center px-4 h-full border-b hover:bg-muted/50 transition-colors"
              >
      ...
      <section>
        ...
        <p class="text-sm text-muted-foreground mb-4">Adjust the container height as needed.</p>
        <div class="border rounded-lg overflow-hidden">
          <sc-virtual-list [items]="items().slice(0, 500)" [itemHeight]="36" height="200px">
            <ng-template let-item let-index="index">
              <div
...
export class VirtualListDemoComponent {
  ...
  readonly items = signal<string[]>(
    Array.from({ length: 10000 }, (_, i) => `Item ${i + 1} - Lorem ipsum dolor sit amet`),
  );
