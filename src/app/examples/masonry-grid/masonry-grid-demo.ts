    ...
    <div class="space-y-8">
      ...
      <section>
        ...
        <p class="text-sm text-muted-foreground mb-4">
          Resize the window to see responsive behavior: 1 column on mobile, 2 on tablet, 3 on
          desktop
        </p>
        <sc-masonry-grid
          [columns]="3"
        ...
        >
          @for (item of items().slice(0, 9); track item.id) {
            <sc-masonry-item>
              <div class="rounded-lg border bg-card p-4" [style.min-height.px]="item.height">
                <h4 class="font-medium">Card {{ item.id }}</h4>
                <p class="text-sm text-muted-foreground mt-2">
                  This card has a minimum height of {{ item.height }}px. The masonry layout
          ...
          @for (card of cards(); track card.id) {
            <sc-masonry-item>
              <div class="rounded-lg border bg-card overflow-hidden">
                @if (card.image) {
                  <img [src]="card.image" [alt]="card.title" class="w-full h-32 object-cover" />
                }
                <div class="p-4">
                  <h4 class="font-semibold">{{ card.title }}</h4>
                  <p class="text-sm text-muted-foreground mt-2">{{ card.description }}</p>
...
export class MasonryGridDemoComponent {
  ...
  readonly images = signal([
    ...
    { id: 6, url: 'https://picsum.photos/seed/m6/400/400', title: 'Street Art', category: 'Urban' },
    {
      id: 7,
  ...
  readonly cards = signal([
    {
      ...
      description: 'Learn the basics of Angular framework and build your first application.',
    ...
    },
    {
      ...
      description: 'How to create and maintain a design system for your organization.',
      image: null,
      tags: ['Design', 'UI/UX'],
    },
    {
    ...
    },
    {
      ...
      description: 'Tips and tricks for optimizing web application performance.',
    ...
    },
    {
      ...
      description: 'Building inclusive web applications that work for everyone.',
      image: null,
      tags: ['A11y', 'Web'],
    },
  ]);
