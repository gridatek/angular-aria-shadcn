    ...
    <div class="space-y-8">
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-col gap-4">
          ...
          <p class="text-sm text-muted-foreground">Selected: {{ formatTime24(time24h()) }}</p>
        </div>
      </div>
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-col gap-4">
          <div sc-time-picker format="24h" [showSeconds]="true" [(value)]="timeWithSeconds">
            <input sc-time-picker-input type="hours" label="Hours" />
            <span sc-time-picker-separator>:</span>
            <input sc-time-picker-input type="minutes" label="Minutes" />
            <span sc-time-picker-separator>:</span>
      ...
      <div class="space-y-4">
        ...
        <div sc-time-picker format="12h" [disabled]="true" [(value)]="disabledTime">
          <input sc-time-picker-input type="hours" label="Hours" />
          <span sc-time-picker-separator>:</span>
          <input sc-time-picker-input type="minutes" label="Minutes" />
          <div sc-time-picker-period></div>
      ...
      <div class="space-y-4">
        ...
        <div class="flex items-start gap-8">
          ...
          <div class="space-y-2">
            ...
            <p class="text-2xl tabular-nums">
              {{ ((clockMinutesTime()?.hours ?? 0) % 12 || 12).toString().padStart(2, '0') }}:{{
...
export class ScTimePickerDemo {
  readonly time12h = signal<TimeValue | null>({ hours: 9, minutes: 30, period: 'AM' });
  readonly time24h = signal<TimeValue | null>({ hours: 14, minutes: 45 });
  readonly timeWithSeconds = signal<TimeValue | null>({ hours: 10, minutes: 30, seconds: 15 });
  readonly disabledTime = signal<TimeValue | null>({ hours: 8, minutes: 0, period: 'AM' });
  readonly clockTime = signal<TimeValue | null>({ hours: 10, minutes: 0 });
  readonly clockMinutesTime = signal<TimeValue | null>({ hours: 10, minutes: 30 });
