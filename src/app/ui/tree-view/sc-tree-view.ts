...
export class ScTree {
  ...
  protected readonly class = computed(() => cn('flex flex-col gap-1', this.classInput()));
}
...
export class ScTreeItem {
  private readonly parentItem = inject(SC_TREE_ITEM, { optional: true, skipSelf: true });
  ...
  protected readonly class = computed(() => cn('flex flex-col', this.classInput()));
...
export class ScTreeItemIcon {
  ...
  protected readonly class = computed(() => cn('size-4 shrink-0', this.classInput()));
}
