...
@Component({
  selector: 'sc-toaster',
  imports: [ScToast, ScToastTitle, ScToastDescription, ScToastClose, ScToastAction],
  template: `
    @for (toast of toastService.toasts(); track toast.id) {
      <div sc-toast [variant]="toast.variant ?? 'default'" [attr.data-state]="'open'">
        ...
        @if (toast.action) {
          <button sc-toast-action (click)="onAction(toast.id, toast.action.onClick)">
            {{ toast.action.label }}
          </button>
        }
