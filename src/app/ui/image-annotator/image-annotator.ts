...
import type { Annotation, AnnotationPoint, AnnotationTool } from './image-annotator-types';
...
export class ScImageAnnotator implements AfterViewInit {
  readonly imageCanvasRef = viewChild<ElementRef<HTMLCanvasElement>>('imageCanvas');
  readonly annotationCanvasRef = viewChild<ElementRef<HTMLCanvasElement>>('annotationCanvas');
  ...
  private loadImage(): void {
    ...
    img.onerror = () => {
      ...
      ctx.fillText('Failed to load image', this.canvasWidth() / 2, this.canvasHeight() / 2);
      this.imageLoaded.set(true);
    };
    img.src = this.src();
  }
  ...
  protected toolButtonClass(tool: AnnotationTool): string {
    return cn(
      ...
      this.currentTool() === tool ? 'bg-primary text-primary-foreground' : 'hover:bg-muted',
    );
  }
  ...
  private drawPen(ctx: CanvasRenderingContext2D, points: AnnotationPoint[]): void {
  ...
  private drawLine(ctx: CanvasRenderingContext2D, points: AnnotationPoint[]): void {
    if (points.length < 2) return;
    ctx.beginPath();
    ctx.moveTo(points[0].x, points[0].y);
  ...
  private drawRectangle(ctx: CanvasRenderingContext2D, points: AnnotationPoint[]): void {
    if (points.length < 2) return;
  ...
  private drawCircle(ctx: CanvasRenderingContext2D, points: AnnotationPoint[]): void {
    if (points.length < 2) return;
  ...
  private drawArrow(ctx: CanvasRenderingContext2D, points: AnnotationPoint[]): void {
    if (points.length < 2) return;
