...
const DEFAULT_TIMEZONES: Timezone[] = [
  ...
  { id: 'America/Los_Angeles', label: 'Pacific Time', offset: '-08:00', abbr: 'PST' },
  { id: 'America/Denver', label: 'Mountain Time', offset: '-07:00', abbr: 'MST' },
  { id: 'America/Chicago', label: 'Central Time', offset: '-06:00', abbr: 'CST' },
  { id: 'America/New_York', label: 'Eastern Time', offset: '-05:00', abbr: 'EST' },
...
export const SC_TIMEZONE_CONFIG = new InjectionToken<TimezoneConfig>('SC_TIMEZONE_CONFIG', {
  providedIn: 'root',
  factory: () => ({
...
export class ScTimezoneService {
  ...
  readonly timezones = signal<Timezone[]>(this.config.timezones ?? DEFAULT_TIMEZONES);
  ...
  formatTime(timezoneId?: string, options?: Intl.DateTimeFormatOptions): string {
    ...
    return new Intl.DateTimeFormat('en-US', { ...defaultOptions, ...options }).format(now);
  }
  ...
  private detectCurrentTimezone(): string {
    ...
    if (this.config.defaultTimezone && this.isValidTimezone(this.config.defaultTimezone)) {
