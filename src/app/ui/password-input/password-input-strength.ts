...
export class ScPasswordInputStrength {
  ...
  readonly requirements = [
    ...
    { label: 'Contains special character', test: (v: string) => /[!@#$%^&*(),.?":{}|<>]/.test(v) },
  ];
  ...
  protected readonly containerClass = computed(() => cn('w-full', this.classInput()));
  ...
  protected requirementClass(met: boolean): string {
    return cn('flex items-center', met ? 'text-green-600' : 'text-muted-foreground');
  }
}
