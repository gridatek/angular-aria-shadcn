...
export const SC_TIME_PICKER = new InjectionToken<ScTimePicker>('SC_TIME_PICKER');
...
export class ScTimePickerInput {
  ...
  protected readonly displayValue = computed(() => {
    ...
    if (type === 'hours') {
      num = this.timePicker.format() === '12h' ? val.hours % 12 || 12 : val.hours;
...
export class ScTimePickerPeriod {
  ...
  protected readonly class = computed(() =>
    cn('ml-2 inline-flex rounded-md border border-input bg-background p-0.5', this.classInput()),
  );
      ...
      <svg viewBox="0 0 200 200" class="size-48">
        ...
        @for (marker of markers(); track marker.value) {
          ...
          >
            <circle
              ...
              [class.fill-accent]="hoveredValue() === marker.value && !isSelected(marker.value)"
        ...
        @if (selectedAngle() !== null) {
          <line
            ...
            stroke-width="2"
          />
          <circle cx="100" cy="100" r="4" fill="currentColor" class="text-primary" />
        }
      </svg>
    </div>
  `,
...
export class ScTimePickerClock {
  ...
  protected readonly selectedAngle = computed(() => {
    ...
    const isHours = this.mode() === 'hours';
    if (isHours) {
      const hours = this.timePicker.format() === '12h' ? val.hours % 12 || 12 : val.hours % 12;
