...
@Component({
  ...
  template: `
    <div [class]="containerClass()">
      ...
      @if (showInfo()) {
        <div [class]="infoClass()">
          <p class="font-medium truncate">{{ currentTrack()?.title || 'Unknown Track' }}</p>
          @if (currentTrack()?.artist) {
            <p class="text-sm text-muted-foreground truncate">{{ currentTrack()?.artist }}</p>
          }
        </div>
      }
      ...
      <div [class]="progressContainerClass()">
        ...
        <span class="text-xs text-muted-foreground w-10">{{ formatTime(duration()) }}</span>
      </div>
...
export class ScAudioPlayer {
  ...
  private readonly audioRef = viewChild.required<ElementRef<HTMLAudioElement>>('audioElement');
  ...
  protected readonly progressContainerClass = computed(() => cn('flex items-center gap-2'));

  protected readonly controlsClass = computed(() => cn('flex items-center justify-center gap-2'));

  protected readonly volumeClass = computed(() => cn('flex items-center gap-2 justify-center'));
  ...
  protected onEnded(): void {
    ...
    } else if (repeatMode === 'all' || this.currentIndex() < this.tracks().length - 1) {
      this.next();
    }
  }
