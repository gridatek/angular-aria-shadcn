...
const LANGUAGES: Record<string, LanguageDefinition> = {
  javascript: {
    ...
    strings: /(['"`])(?:(?!\1)[^\\]|\\.)*?\1/,
    comments: { line: '//', block: ['/*', '*/'] },
    numbers: /\b(?:0x[\da-fA-F]+|0b[01]+|0o[0-7]+|\d+(?:\.\d+)?(?:e[+-]?\d+)?)\b/i,
  ...
  python: {
    ...
    operators: /[+\-*/%=<>!&|^~@]+|:=/,
    strings: /(?:f|r|b|fr|rf|br|rb)?(['"])(?:(?!\1)[^\\]|\\.)*?\1|'''[\s\S]*?'''|"""[\s\S]*?"""/,
    comments: { line: '#' },
    numbers: /\b(?:0x[\da-fA-F]+|0b[01]+|0o[0-7]+|\d+(?:\.\d+)?(?:e[+-]?\d+)?j?)\b/i,
    functions: /\b([a-zA-Z_][\w]*)\s*(?=\()/,
  },
  html: {
    keywords: [],
...
export function tokenize(code: string, language: Language): Token[] {
  ...
  while (remaining.length > 0) {
    ...
    if (lang.comments.line && remaining.startsWith(lang.comments.line)) {
      ...
      matched = true;
    } else if (lang.comments.block && remaining.startsWith(lang.comments.block[0])) {
      const endIdx = remaining.indexOf(lang.comments.block[1], lang.comments.block[0].length);
      const comment =
        endIdx === -1 ? remaining : remaining.slice(0, endIdx + lang.comments.block[1].length);
      tokens.push({ type: 'comment', value: comment });
      remaining = remaining.slice(comment.length);
      matched = true;
    }
    ...
    // Check for strings
    const stringMatch = remaining.match(new RegExp(`^${lang.strings.source}`, lang.strings.flags));
    if (stringMatch) {
    ...
    // Check for numbers
    const numMatch = remaining.match(new RegExp(`^${lang.numbers.source}`, lang.numbers.flags));
    if (numMatch) {
    ...
    // Check for functions
    if (lang.functions) {
      const funcMatch = remaining.match(new RegExp(`^${lang.functions.source}`));
      if (funcMatch) {
    ...
    if (identMatch) {
      ...
      if (lang.keywords.includes(word) || lang.keywords.includes(word.toLowerCase())) {
        type = 'keyword';
...
export function detectLanguage(code: string, filename?: string): Language {
  ...
  if (code.includes('<!DOCTYPE') || code.includes('<html') || /<\w+[^>]*>/.test(code)) {
    return 'html';
  }
  ...
  if (/^(import|export|const|let|var|function|class)\s/.test(code)) {
    if (/:\s*(string|number|boolean|any|void|never)\b/.test(code) || /interface\s+\w+/.test(code)) {
      return 'typescript';
    }
    return 'javascript';
  }
  if (/^(def|class|import|from|if __name__)\s/.test(code) || /:\s*$/.test(code.split('\n')[0])) {
    return 'python';
  }
