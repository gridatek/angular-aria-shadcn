...
@Component({
  ...
  template: `
    <div [class]="containerClass()" [style]="containerStyle()" (click)="focusTextarea()">
      <!-- Header bar -->
      @if (showHeader()) {
        <div class="flex items-center justify-between px-3 py-2 border-b bg-background/50">
          ...
          >
            @for (line of lines(); track $index) {
              <div
                ...
                [style.color]="
                  activeLine() === $index + 1 ? theme().lineNumbersActive : theme().lineNumbers
                "
              >
                {{ $index + 1 }}
              </div>
...
export class ScCodeEditor {
  ...
  protected onKeydown(event: KeyboardEvent): void {
    ...
    if (event.key === 'Enter' && !event.shiftKey) {
      ...
      event.preventDefault();
      const newValue =
        value.slice(0, start) + '\n' + currentIndent + extraIndent + value.slice(start);
    ...
    if (event.key === '}' || event.key === ']' || event.key === ')') {
      ...
      if (/^\s+$/.test(beforeCursor)) {
        ...
        event.preventDefault();
        const newValue = value.slice(0, lineStart) + newIndent + event.key + value.slice(start);

        this.value.set(newValue);
        this.valueChange.emit(newValue);

        setTimeout(() => {
          target.selectionStart = target.selectionEnd = lineStart + newIndent.length + 1;
        });
      }
    }
  }
