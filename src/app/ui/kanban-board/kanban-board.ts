...
export class ScKanbanBoard {
  ...
  protected onCardMoved(event: KanbanDragEvent): void {
    ...
    const otherCards = cards.filter(
      (c) => c.columnId !== event.targetColumnId && c.columnId !== event.sourceColumnId,
    );
  ...
  protected submitNewColumnFromButton(): void {
    const input = document.querySelector('sc-kanban-board input[type="text"]') as HTMLInputElement;
    const title = input?.value.trim();
