...
@Component({
  ...
  template: `
    <div [class]="columnClass()">
      ...
      <!-- Collapsed state -->
      @if (collapsed()) {
        <div class="p-3 text-center text-sm text-muted-foreground">{{ cards().length }} cards</div>
      } @else {
        <!-- Cards container -->
        <div
...
export class ScKanbanColumn {
  ...
  protected startAddingCard(): void {
    ...
    requestAnimationFrame(() => {
      const input = document.querySelector('sc-kanban-column textarea') as HTMLTextAreaElement;
      input?.focus();
    });
  }
  ...
  protected submitNewCardFromButton(): void {
    const textarea = document.querySelector('sc-kanban-column textarea') as HTMLTextAreaElement;
    const title = textarea?.value.trim();
