...
@Component({
  ...
  template: `
    <div
      ...
      [style.column-count]="layoutMode() === 'columns' ? currentColumns() : null"
...
export class ScMasonryGrid implements AfterViewInit {
  ...
  protected readonly containerClass = computed(() => cn('w-full', this.class()));

  constructor() {
    // Set CSS variable for gap
    this.elementRef.nativeElement.style.setProperty('--masonry-gap', `${this.gap()}px`);
  }
