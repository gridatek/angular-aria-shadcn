...
export class ScVideoPlayer {
  ...
  private readonly videoRef = viewChild.required<ElementRef<HTMLVideoElement>>('videoElement');
  ...
  protected readonly controlsContainerClass = computed(() =>
    cn(
      ...
      this.showControlsComputed() ? 'opacity-100' : 'opacity-0 pointer-events-none',
    ),
  );
