...
import type { SplitButtonAction, SplitButtonSize, SplitButtonVariant } from './split-button-types';
...
export class ScSplitButton implements AfterViewInit {
  ...
  protected readonly containerClass = computed(() => cn('inline-flex', this.class()));
  ...
  protected readonly dropdownMenuClass = computed(() =>
    cn(
      ...
      this.size() === 'sm' ? 'top-8' : this.size() === 'lg' ? 'top-12' : 'top-10',
    ),
  );
