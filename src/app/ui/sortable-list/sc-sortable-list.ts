...
export const SC_SORTABLE_LIST = new InjectionToken<ScSortableList<unknown>>('SC_SORTABLE_LIST');

// Token for sortable item context
export const SC_SORTABLE_ITEM = new InjectionToken<ScSortableItem<unknown>>('SC_SORTABLE_ITEM');
...
export class ScSortableList<T> {
  ...
  unregisterItem(index: number): void {
    this.registeredItems = this.registeredItems.filter((item) => item.index !== index);
  }
...
export class ScSortableItem<T> {
  ...
  protected readonly isBeingDragged = computed(() => this.list.draggedIndex() === this.index());
...
@Component({
  ...
  template: `
    @if (list.isDragging()) {
      <div class="fixed inset-0 z-40 cursor-grabbing" (dragover)="onDragOver($event)"></div>
    }
  `,
  host: {
