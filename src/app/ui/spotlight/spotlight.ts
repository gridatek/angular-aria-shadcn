      ...
      >
        ...
        @if (targetRect()) {
          ...
          >
            ...
            ></div>
            <!-- Solid border -->
            <div class="absolute inset-0 rounded-[inherit] border-2 border-primary"></div>
          </div>
        }
        ...
        @if (targetRect()) {
          <div [class]="contentClass()" [style]="contentStyle()" (click)="$event.stopPropagation()">
...
export class ScSpotlight {
  ...
  readonly contentPlacement = input<'top' | 'bottom' | 'left' | 'right' | 'auto'>('auto');
  ...
  protected readonly containerClass = computed(() =>
    cn('fixed inset-0 z-[9999]', 'animate-in fade-in-0 duration-200', this.class()),
  );
...
export class ScSpotlightTitle {
  ...
  protected readonly computedClass = computed(() => cn('font-semibold text-lg mb-2', this.class()));
}
