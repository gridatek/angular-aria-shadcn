...
import type { Notification, NotificationAction, NotificationGroup } from './notification-types';
          ...
          <div class="p-2 space-y-4">
            @for (group of groups(); track group.id) {
              <sc-notification-group
                ...
                (actionClick)="onActionClick($event.notification, $event.action)"
            ...
            @if (ungroupedNotifications().length > 0) {
              <div>
                <p class="px-3 py-2 text-sm font-medium text-muted-foreground">Other</p>
                <div class="space-y-1">
                  @for (notification of ungroupedNotifications(); track notification.id) {
...
export class ScNotificationCenter {
  ...
  readonly actionClick = output<{ notification: Notification; action: NotificationAction }>();
  ...
  protected onMarkAllReadInGroup(groupId: string): void {
    this.notifications.update((notifications) =>
      notifications.map((n) => (n.groupId === groupId ? { ...n, read: true } : n)),
    );
  }
