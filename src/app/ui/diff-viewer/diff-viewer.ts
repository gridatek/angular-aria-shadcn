...
import { computeDiff, computeWordDiff, DiffLine, DiffResult } from './diff-algorithm';
        ...
        <div class="flex items-center justify-between px-4 py-2 border-b bg-muted/30">
          ...
          <div class="flex items-center gap-4">
            ...
            <div class="flex items-center gap-3 text-sm">
              ...
              <span class="text-red-600 dark:text-red-400"> -{{ diffResult().deletions }} </span>
            </div>
            <!-- View mode toggle -->
                ...
                @for (line of diffResult().lines; track $index) {
                  @if (line.type !== 'added') {
                    <div [class]="getLineClass(line.type, 'old')">
                      ...
                      <span class="px-2" [innerHTML]="highlightLine(line, 'old')"></span>
                    </div>
                  } @else {
                ...
                @for (line of diffResult().lines; track $index) {
                  @if (line.type !== 'removed') {
                    <div [class]="getLineClass(line.type, 'new')">
                      ...
                      <span class="px-2" [innerHTML]="highlightLine(line, 'new')"></span>
                    </div>
                  } @else {
                    <div class="h-6 bg-muted/30"></div>
                  }
          ...
          <div class="font-mono text-sm">
            @for (line of diffResult().lines; track $index) {
              <div [class]="getLineClass(line.type, 'unified')">
                ...
                <span class="inline-block w-6 text-center select-none font-bold">
                  @switch (line.type) {
                    @case ('added') {
                ...
                <span
                  class="px-2"
                  [innerHTML]="highlightLine(line, line.type === 'removed' ? 'old' : 'new')"
                ></span>
              </div>
            }
          </div>
...
export class ScDiffViewer {
  ...
  protected getLineClass(type: string, side: 'old' | 'new' | 'unified'): string {
    ...
    switch (type) {
      case 'added':
        return cn(baseClass, 'bg-green-500/10 text-green-900 dark:text-green-100');
      case 'removed':
  ...
  protected highlightLine(line: DiffLine, side: 'old' | 'new'): string {
    ...
    if (line.oldContent !== undefined && line.newContent !== undefined) {
      const { oldParts, newParts } = computeWordDiff(line.oldContent, line.newContent);
