...
export class ScScrollBar implements OnInit {
  ...
  protected readonly thumbSize = computed(() => {
    if (this.orientation() === 'vertical') {
      const ratio = this.scrollArea.clientHeight() / this.scrollArea.scrollHeight();
  ...
  protected readonly thumbPosition = computed(() => {
    if (this.orientation() === 'vertical') {
      const scrollableHeight = this.scrollArea.scrollHeight() - this.scrollArea.clientHeight();
      if (scrollableHeight <= 0) return 0;
      const trackHeight = 100 - this.thumbSize();
      return (this.scrollArea.scrollTop() / scrollableHeight) * trackHeight;
    }
    const scrollableWidth = this.scrollArea.scrollWidth() - this.scrollArea.clientWidth();
  ...
  protected readonly thumbClass = computed(() => cn('relative flex-1 rounded-full bg-border'));
  ...
  ngOnInit(): void {
    ...
    merge(fromEvent<MouseEvent>(document, 'mousemove'), fromEvent<MouseEvent>(document, 'mouseup'))
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe((event) => {
  ...
  protected onTrackClick(event: MouseEvent): void {
    ...
    if (this.orientation() === 'vertical') {
      ...
      const scrollableWidth = this.scrollArea.scrollWidth() - this.scrollArea.clientWidth();
      this.scrollArea.scrollTo(this.scrollArea.scrollTop(), clickPosition * scrollableWidth);
    }
  }
  ...
  private onMouseMove(event: MouseEvent): void {
    ...
    if (this.orientation() === 'vertical') {
      ...
      this.scrollArea.scrollTo(this.scrollArea.scrollTop(), this.dragStartScroll + scrollDelta);
    }
  }
}
