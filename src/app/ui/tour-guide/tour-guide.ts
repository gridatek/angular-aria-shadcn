    ...
    @if (tourService.isActive()) {
      ...
      >
        ...
        @if (targetRect()) {
          ...
          >
            <div class="absolute inset-0 rounded-lg animate-pulse bg-primary/10"></div>
          </div>
        }
        ...
        @if (currentStep()) {
          ...
          >
            ...
            <!-- Content -->
            <div class="pr-6">
              <h3 class="font-semibold text-lg mb-2">{{ currentStep()!.title }}</h3>
              <p class="text-sm text-muted-foreground">{{ currentStep()!.content }}</p>
            </div>
            ...
            <div class="flex items-center justify-between mt-4 pt-4 border-t">
              <div class="text-sm text-muted-foreground">
                {{ tourService.currentStep() + 1 }} of {{ tourService.steps().length }}
              </div>
...
export class ScTourGuide {
  ...
  protected readonly currentStep = computed(() => this.tourService.currentStepData());

  protected readonly overlayOpacity = computed(
    () => this.tourService.options().overlayOpacity ?? 0.5,
  );

  protected readonly highlightPadding = computed(() => this.currentStep()?.highlightPadding ?? 8);

  protected readonly showProgress = computed(() => this.tourService.options().showProgress ?? true);

  protected readonly showStepNumbers = computed(
    () => this.tourService.options().showStepNumbers ?? true,
  );

  protected readonly allowClose = computed(() => this.tourService.options().allowClose ?? true);
  ...
  updateTargetRect(): void {
    ...
    const scrollBehavior = this.tourService.options().scrollBehavior ?? 'smooth';

    const elementTop = rect.top;
    const elementBottom = rect.bottom;
    const viewportHeight = window.innerHeight;

    if (elementTop < scrollPadding || elementBottom > viewportHeight - scrollPadding) {
  ...
  protected onKeydown(event: KeyboardEvent): void {
    const allowKeyboard = this.tourService.options().allowKeyboardNavigation ?? true;
