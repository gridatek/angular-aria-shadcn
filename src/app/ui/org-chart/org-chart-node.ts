    ...
    <div [class]="containerClass()">
      ...
      <div [class]="nodeCardClass()">
        <button
          ...
          [attr.aria-label]="node().name + (node().title ? ', ' + node().title : '')"
        ...
        >
          ...
          <div class="flex-1 min-w-0 text-left">
            ...
            @if (node().title) {
              <p class="text-xs text-muted-foreground truncate">{{ node().title }}</p>
            }
            @if (node().department) {
              <p class="text-xs text-muted-foreground/70 truncate">{{ node().department }}</p>
            }
          </div>
...
export class ScOrgChartNode {
  ...
  protected readonly containerClass = computed(() =>
    cn('flex', this.direction() === 'vertical' ? 'flex-col items-center' : 'flex-row items-start'),
  );
  ...
  protected readonly expandIconClass = computed(() =>
    cn(
      ...
      this.direction() === 'horizontal' && (this.isExpanded() ? '-rotate-90' : 'rotate-90'),
    ),
  );
  ...
  protected readonly childrenWrapperClass = computed(() =>
    cn('flex relative', this.direction() === 'vertical' ? 'flex-row gap-4' : 'flex-col gap-4'),
  );

  protected childNodeClass(isFirst: boolean, isLast: boolean): string {
    return cn(
      'flex relative',
      this.direction() === 'vertical' ? 'flex-col items-center' : 'flex-row items-start',
    );
  }
