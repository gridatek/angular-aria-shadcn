      ...
      <div [class]="listContainerClass()">
        ...
        <div [class]="listClass()">
          ...
          @if (filteredSourceItems().length === 0) {
            <div class="p-4 text-center text-sm text-muted-foreground">No items</div>
          }
        </div>
      </div>
...
export class ScTransferList {
  ...
  readonly change = output<{ source: TransferListItem[]; target: TransferListItem[] }>();
  ...
  protected readonly allSourceSelected = computed(() => {
    ...
    return items.length > 0 && items.every((i) => this.selectedSourceIds().has(i.id));
  });
  ...
  protected readonly allTargetSelected = computed(() => {
    ...
    return items.length > 0 && items.every((i) => this.selectedTargetIds().has(i.id));
  });
  ...
  moveToTarget(): void {
    ...
    const toMove = this.sourceItems().filter((i) => selectedIds.has(i.id) && !i.disabled);
    const remaining = this.sourceItems().filter((i) => !selectedIds.has(i.id) || i.disabled);
  ...
  moveToSource(): void {
    ...
    const toMove = this.targetItems().filter((i) => selectedIds.has(i.id) && !i.disabled);
    const remaining = this.targetItems().filter((i) => !selectedIds.has(i.id) || i.disabled);
