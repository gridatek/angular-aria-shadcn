...
export class ScInfiniteScroll implements AfterViewInit {
  ...
  protected readonly containerClass = computed(() => cn('overflow-auto', this.classInput()));
  ...
  private setupIntersectionObserver(): void {
    ...
    const container =
      this.elementRef.nativeElement.querySelector('[data-slot="infinite-scroll"] > div') ||
    ...
    this.observer = new IntersectionObserver(
      (entries) => {
        ...
        if (entry.isIntersecting && !this.disabled() && !this.loading() && !this.hasReachedEnd()) {
          this.loadMore.emit();
        }
      },
      {
...
export class ScVirtualScroll<T> {
  ...
  protected readonly viewportClass = computed(() => cn('overflow-auto', this.classInput()));
