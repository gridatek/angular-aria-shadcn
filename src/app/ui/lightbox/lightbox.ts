  ...
  template: `
    @if (isOpen()) {
      <div
        ...
        aria-modal="true"
        [attr.aria-label]="
          'Image gallery, showing image ' + (currentIndex() + 1) + ' of ' + images().length
        "
      >
        <!-- Close button -->
        <button
        ...
        <div [class]="bottomBarClass()">
          <!-- Image info -->
          @if (showInfo() && (currentImage()?.title || currentImage()?.description)) {
            <div class="flex-1 text-white">
              @if (currentImage()?.title) {
                <h3 class="font-semibold">{{ currentImage()?.title }}</h3>
              }
              @if (currentImage()?.description) {
                <p class="text-sm text-white/80">{{ currentImage()?.description }}</p>
              }
            </div>
          }
...
export class ScLightbox {
  ...
  protected readonly overlayClass = computed(() =>
    cn('fixed inset-0 z-50 bg-black/95 flex flex-col', 'animate-in fade-in-0 duration-200'),
  );
  ...
  protected readonly thumbnailsClass = computed(() =>
    cn('flex items-center justify-center gap-2 px-4 py-3 bg-black/50 overflow-x-auto'),
  );
  ...
  protected thumbnailClass(index: number): string {
    return cn(
      ...
      index === this.currentIndex() ? 'ring-white' : 'ring-transparent hover:ring-white/50',
    );
  }
...
export class ScLightboxGallery {
  ...
  protected readonly galleryClass = computed(() =>
    cn('grid', `grid-cols-${this.columns()}`, `gap-${this.gap()}`, this.classInput()),
  );
