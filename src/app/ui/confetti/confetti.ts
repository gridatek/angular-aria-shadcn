...
import type { ConfettiOptions, ConfettiParticle, ConfettiShape } from './confetti-types';
...
export class ScConfetti implements AfterViewInit {
  ...
  fire(customOptions?: ConfettiOptions): void {
    const opts = { ...DEFAULT_CONFETTI_OPTIONS, ...this.options(), ...customOptions };
  ...
  private createParticles(opts: Required<ConfettiOptions>): void {
    ...
    for (let i = 0; i < opts.particleCount; i++) {
      const angle = (Math.random() * opts.spread - opts.spread / 2) * (Math.PI / 180);
