...
export class ScRatingItem {
  ...
  protected readonly isSelected = computed(() => this.rating.value() >= this.value());
  ...
  protected readonly state = computed(() => {
    ...
    if (this.rating.allowHalf() && displayValue >= itemValue - 0.5) return 'half';
    return 'empty';
  });
...
export class ScRatingStars {
  ...
  protected readonly filledStarClass = computed(() => cn(this.starClass(), this.color()));
}
