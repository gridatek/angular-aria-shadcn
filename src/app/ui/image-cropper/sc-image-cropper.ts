...
export const SC_IMAGE_CROPPER = new InjectionToken<ScImageCropper>('SC_IMAGE_CROPPER');
...
export class ScImageCropper implements AfterViewInit, OnDestroy {
  ...
  readonly outputType = input<'image/png' | 'image/jpeg' | 'image/webp'>('image/png');
  readonly outputQuality = input<number>(0.92);

  readonly cropArea = model<CropArea>({ x: 50, y: 50, width: 200, height: 200 });
  ...
  protected readonly scaledImageWidth = computed(() => this.imageNaturalWidth() * this.zoom());
  protected readonly scaledImageHeight = computed(() => this.imageNaturalHeight() * this.zoom());
  ...
  private startResizing(clientX: number, clientY: number, handle: string): void {
  ...
  private handleResize(clientX: number, clientY: number): void {
    ...
    switch (this.resizeHandle) {
      case 'se':
        ...
        newH = aspectRatio !== null ? newW / aspectRatio : Math.max(minH, crop.height + deltaY);
        break;
      case 'sw':
        newW = Math.max(minW, crop.width - deltaX);
        newH = aspectRatio !== null ? newW / aspectRatio : Math.max(minH, crop.height + deltaY);
...
export class ScImageCropperControls {
  ...
  protected readonly class = computed(() => cn('flex items-center gap-4 py-2', this.classInput()));
...
@Component({
  ...
  template: `
    <div class="overflow-hidden bg-muted" [style.width.px]="width()" [style.height.px]="height()">
      <img
        [src]="cropper.src()"
        class="max-w-none"
