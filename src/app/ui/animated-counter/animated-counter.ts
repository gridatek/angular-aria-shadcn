...
export class ScAnimatedCounter {
  ...
  readonly easing = input<AnimatedCounterEasing>(DEFAULT_COUNTER_OPTIONS.easing);
  ...
  protected readonly containerClass = computed(() => cn('tabular-nums font-medium', this.class()));
  ...
  private getEasingFunction(easing: AnimatedCounterEasing): (t: number) => number {
    switch (easing) {
      case 'linear':
        return (t) => t;
      case 'easeIn':
  ...
  private formatNumber(value: number): string {
    ...
    const withSeparator = intPart.replace(/\B(?=(\d{3})+(?!\d))/g, this.separator());
