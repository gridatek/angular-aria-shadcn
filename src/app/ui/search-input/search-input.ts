...
@Component({
  ...
  imports: [FormsModule],
  template: `
    <div [class]="containerClass()" role="combobox" [attr.aria-expanded]="isOpen()">
      ...
      @if (isOpen() && filteredSuggestions().length > 0) {
        <div id="search-suggestions" [class]="suggestionsClass()" role="listbox">
          @for (suggestion of filteredSuggestions(); track suggestion.id; let i = $index) {
            ...
            >
              ...
              @if (suggestion.category) {
                <span class="text-xs text-muted-foreground">{{ suggestion.category }}</span>
              }
            </button>
          }
        </div>
      ...
      @if (
        isOpen() && query().length >= minChars() && filteredSuggestions().length === 0 && !loading()
      ) {
...
export class ScSearchInput {
  ...
  protected readonly containerClass = computed(() => cn('relative', this.class()));
