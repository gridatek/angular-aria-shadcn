...
function encodeData(data: string, version: number, ecLevel: ErrorCorrectionLevel): number[] {
...
function placeData(matrix: (boolean | null)[][], codewords: number[], ecCodewords: number[]): void {
  const size = matrix.length;
...
export function generateQRCode(data: string, ecLevel: ErrorCorrectionLevel = 'M'): boolean[][] {
    ...
    if (version === 2) {
      ...
      for (const r of positions) {
        for (const c of positions) {
          if ((r === 6 && c === 6) || (r === 6 && c === alignPos) || (r === alignPos && c === 6)) {
            continue;
          }
