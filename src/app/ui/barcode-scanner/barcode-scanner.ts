      ...
      @if (!isSupported()) {
        <div class="flex flex-col items-center justify-center p-8 text-center">
          ...
          <p class="text-sm text-muted-foreground">
            Your browser doesn't support the Barcode Detection API. Try using Chrome, Edge, or Opera
            on desktop/Android.
          </p>
        </div>
      } @else if (error()) {
        ...
        <!-- Video Preview -->
        <div class="relative">
          <video #videoElement [class]="videoClass()" autoplay playsinline muted></video>

          <!-- Scanning Overlay -->
          @if (isScanning()) {
            <div class="absolute inset-0 pointer-events-none">
              <!-- Corner markers -->
              <div class="absolute inset-8 border-2 border-primary/50 rounded-lg">
                <div
          ...
          @if (lastResult() && showLastResult()) {
            <div class="absolute bottom-4 left-4 right-4 bg-black/80 text-white p-3 rounded-lg">
              <p class="text-xs text-white/70 mb-1">{{ lastResult()?.format }}</p>
              <p class="font-mono text-sm break-all">{{ lastResult()?.rawValue }}</p>
            </div>
          }
        </div>
...
export class ScBarcodeScanner {
  ...
  protected readonly videoClass = computed(() => cn('w-full aspect-video object-cover bg-muted'));
  ...
  private async detectBarcode(): Promise<void> {
    const video = this.videoRef()?.nativeElement;
    if (!video || !this.detector || video.readyState !== video.HAVE_ENOUGH_DATA) return;
