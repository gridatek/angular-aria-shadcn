...
export const SC_LANGUAGE_CONFIG = new InjectionToken<LanguageConfig>('SC_LANGUAGE_CONFIG', {
  providedIn: 'root',
  factory: () => ({
    languages: [
...
export class ScLanguageService {
  ...
  private detectLanguageFromUrl(): string | null {
    ...
    const language = this.languages().find((lang) => lang.code === firstSegment);
  ...
  private defaultUrlResolver(languageCode: string, currentUrl: string): string {
    ...
    if (currentLangCode) {
      ...
      const regex = new RegExp(`^(${basePath.replace(/\/$/, '')}/)${currentLangCode}(/|$)`);
      ...
      const pathWithoutBase = pathname.startsWith(basePath)
        ? pathname.slice(basePath.length)
        : pathname.slice(1);
      url.pathname = `${basePath.replace(/\/$/, '')}/${languageCode}/${pathWithoutBase}`.replace(
        /\/+/g,
        '/',
      );
    }
