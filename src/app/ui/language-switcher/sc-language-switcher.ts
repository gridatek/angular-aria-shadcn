...
@Component({
  ...
  template: `
    ...
    @if (!iconOnly()) {
      <span class="text-xs font-semibold uppercase">{{ currentLanguageCode() }}</span>
    }
  `,
...
export class ScLanguageToggle {
  ...
  protected readonly currentLanguageCode = computed(() => this.languageService.language());
...
export class ScLanguageSelect {
  ...
  protected readonly hostClass = computed(() => cn('inline-block', this.classInput()));
